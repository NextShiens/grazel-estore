...
const CustomersDetails = () => {
  ...
  useEffect(() => {
    (async () => {
      singleOrder.forEach(async (order) => {
        ...
        setOrderTracking((prev) => [...prev, data.order]);
      });
    })();
  }, [singleOrder]);
  ...
  function totalCostOrder() {
    return singleOrder
      ...
      .reduce(
        (acc, pro) =>
          acc + pro.discount ? pro.discounted_price : pro.price * pro.quantity,
        0
      );
  }
  function completedOrder() {
    return orderTrackings?.filter(
      (item) => item.status_history.slice(-1)[0]["status"] === "completed"
    )?.length;
  }
  function cancelledOrder() {
    return orderTrackings?.filter(
      (item) => item.status_history.slice(-1)[0]["status"] === "cancelled"
    )?.length;
  }
        ...
        <div className="flex-1 mt-[30px] px-[22px] flex flex-col xl:flex-row gap-5">
          ...
          <div className="xl:w-[320px] bg-white rounded-[8px] shadow-sm flex items-center flex-col px-[20px] py-[39px] h-full">
            <div className="w-[65px] h-[65px] rounded-full bg-[#E0D5C9] overflow-hidden">
              {singleOrder[0]?.customer.image && (
                <Image
                  width={100}
                  height={100}
                  alt=""
                  src={singleOrder[0]?.customer?.image}
            ...
            <div className="w-[100%] flex flex-col gap-2 mt-4">
              ...
              <p className="font-[500] text-[15px]">
                {singleOrder?.[0]?.customer_address?.recipient_phone}
              </p>
            </div>
            <div className="w-[100%] flex flex-col gap-2 mt-4">
            ...
            <div className="w-[100%] flex flex-col gap-2 mt-4">
              ...
              <p className="font-[500] text-[15px]">
                {singleOrder?.[0]?.customer_address?.address}
              </p>
            </div>
          </div>
        </div>
